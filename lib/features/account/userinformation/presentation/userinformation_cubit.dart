...
class UserInformationCubit extends BaseCubit<UserInformationState> {
  ...
  Future<void> init() async {
    ...
    emit(state.copyWith(
      ...
      dtBirthday: state.dateFormat.parse(controller.currentUser.value.birth),
      userAvatar: controller.currentUser.value.avatarUrl,
    ));
  ...
  Future<void> changeDtBirthday(DateTime dt) async {
    state.setAttributes(dtBirthday: dt);
  }
  ...
  Future<void> setValueAddress(String value) async {
    ...
    String tempProvincial = '';
    String tempAdd = '';
    if (Provincial().names.contains(values[values.length - 1].trim())) {
      tempProvincial = values[values.length - 1].trim();
    }
    tempAdd = values[0];
    for (int i = 1; i < values.length - 1; i++) {
      tempAdd = "$tempAdd, ${values[i]}";
    }
    emit(state.copyWith(address: tempAdd, provincial: tempProvincial));
  }
  ...
  String getBirthday() {
    return state.dateFormat.format(state.dtBirthday);
  }
