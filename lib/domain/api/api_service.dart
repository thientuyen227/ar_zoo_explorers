...
class ApiService {
  ...
  Future<String> getImageToTextResponse(File imageFile) async {
  ...
  Future<List<ChatBoxEntity>> getTextToImageResponse(String text) async {
    var request = http.MultipartRequest('POST', Uri.parse(apiUrl));
    request.fields['data'] =
    ...
    if (response.statusCode == 200) {
      ...
      try {
        ...
        List<ChatBoxEntity> chatBoxEntities = [];
        for (var item in parsedData) {
          chatBoxEntities.add(ChatBoxEntity.fromJson(item));
        }
        return chatBoxEntities;
      } catch (e) {
